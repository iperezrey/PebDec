...
for name in list_images:
    ...
    for pred_iou_thresh in np.linspace(0, 1, 10, endpoint=False):

        print(name, pred_iou_thresh)
        ...
        mask_generator_ = SamAutomaticMaskGenerator(
            ...
            points_per_side=32,
            points_per_batch=64,
            pred_iou_thresh=pred_iou_thresh, # The lower the threshold the more objects will pick up
        ...
        plt.axis('off')
        plt.savefig(f'results/{name[4:8]}/IMG_{name[4:8]}_{pred_iou_thresh}_masked.JPEG')
        plt.close()
        ...
        total_area = show_anns(masks, color_by_size=True)
        plt.axis('off')
        plt.savefig(f'results/{name[4:8]}/IMG_{name[4:8]}_{pred_iou_thresh}_masks.JPEG')
        plt.close()
        ...
        # plt.show()
        plt.savefig(f'results/{name[4:8]}/IMG_{name[4:8]}_{pred_iou_thresh}_histogram.JPEG')
        plt.close()
